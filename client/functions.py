...
from .models import ClientUpload, UserProfileUpload
...
def fileuploader(request, file_name, client_profile):
    print('CAME TO UPLOADER')
    print(f'file_name: {file_name}')
    upload_type = f'{file_name}'.upper()
    fhandle = request.FILES[f'{file_name}']
    fs_instance = FileSystemStorage()
    renamed = f'{client_profile.first_name}_{client_profile.last_name}_{upload_type}_{fhandle.name}'
    ...
    ClientUpload.objects.create(
        ...
        upload_file_url=file_url,
        description=upload_type,
    )
    print(f'TOTALY{db_name}')
    messages.success(request, f'{upload_type} uploaded successfully...')
    return 

##### CLIENT FILE UPLOAD HANDLER
def fileuploader_records(request, file_name, user_profile):
    print('CAME TO UPLOADER')
    print(f'file_name: {file_name}')
    upload_type = f'{file_name}'.upper()
    fhandle = request.FILES[f'{file_name}']
    ...
    UserProfileUpload.objects.create(
